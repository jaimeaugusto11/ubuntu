...
const PROVIDERS: Provider[] = [
  {
    ...
    distanceKm: 3.2,
    priceFrom: 3500,
    specialties: ["Limpeza residencial", "Pós-obra"],
    portfolio: [
      { title: "Apartamento T2", img: "" },
      { title: "Escritório 120m²", img: "" },
    ],
  },
  {
    id: "p2",
    ...
    distanceKm: 6.1,
    priceFrom: 8000,
    specialties: ["Reparação de eletrodomésticos", "Instalações"],
  },
  {
    id: "p3",
    name: "Design&Web Pro",
...
const SERVICES: Service[] = [
  {
    id: "s1",
    title: "Limpeza residencial — pacote semanal",
    providerId: "p1",
    estDelivery: "Jun 23 – Jun 24",
    tags: ["limpeza", "residencial"],
  },
  {
    id: "s2",
    title: "Instalação de fogão / exaustor",
    providerId: "p2",
    estDelivery: "Amanhã",
    tags: ["reparação", "instalação"],
  },
  {
    id: "s3",
    title: "Website institucional (até 6 páginas)",
    providerId: "p3",
    estDelivery: "Jul 2",
    tags: ["website", "design"],
  },
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <div
        className={`w-full ${maxWidth} bg-white/90 rounded-2xl shadow-2xl border border-white/60`}
      >
        ...
        {actions && (
          <div className="p-4 border-t border-gray-100/70 flex gap-3 justify-end">
            {actions}
          </div>
        )}
      </div>
...
function ServiceCard({
  ...
  s: Service;
  p: Provider;
  onOpen: (which: ModalKey | null) => void;
}) {
  return (
    <div className="relative rounded-2xl p-4 bg-white/80 backdrop-blur shadow-xl border border-white/60 hover:shadow-2xl hover:-translate-y-1 transition-all">
...
export default function ServicosPage({
  ...
  const servicesFiltered = useMemo(() => {
    return SERVICES.filter((s) => {
      ...
      const matchText =
        ...
        prov.name.toLowerCase().includes(text) ||
        prov.specialties.some((sp) => sp.toLowerCase().includes(text));
      ...
      const matchPrice =
        prov.priceFrom >= priceRange[0] && prov.priceFrom <= priceRange[1];
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 border border-gray-300 bg-white/80 px-4 py-2 rounded-xl shadow-sm hover:shadow-md hover:bg-white transition"
          >
        ...
        <div className="grid grid-cols-12 gap-6 lg:gap-8">
          ...
          <aside className="col-span-12 lg:col-span-3 bg-white/70 backdrop-blur rounded-2xl shadow-xl border border-white/60 p-5 h-fit">
            ...
            <div className="relative mb-4">
              ...
              <input
                ...
                type="text"
                placeholder="Serviço, prestador, especialidade..."
                className="w-full border border-gray-200 bg-gray-50 pl-10 pr-3 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <select
                ...
                className="border border-gray-200 bg-gray-50 px-3 py-2.5 rounded-xl"
              >
                <option>Todas</option>
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="w-full border border-gray-200 bg-gray-50 px-3 py-2.5 rounded-xl"
                  placeholder="Mín."
                />
                <input
                  type="number"
            ...
            <div className="space-y-3">
              {["Entrega", "Método de pagamento", "Condição", "Peso"].map(
                (f) => (
          ...
          {/* Lista de serviços */}
          <section className="col-span-12 lg:col-span-9 grid grid-cols-3 sm:grid-cols-2 xl:grid-cols- gap-4">
            ...
            {servicesFiltered.map((s) => {
              ...
              const onOpen = (which: ModalKey) => {
                if (which !== null) {
                  openAction(s, p, which);
                }
              };
      ...
      {/* Solicitar Orçamento */}
      <Modal
        open={modal === "quote" && !!activeService && !!activeProvider}
        onClose={() => setModal(null)}
        title={`Solicitar orçamento — ${activeService?.title}`}
        actions={
          <>
            <button
      ...
      >
        <div className="space-y-3">
          ...
          <textarea
            ...
            rows={4}
            placeholder="Descreva o serviço, datas, materiais, local, etc."
          />
          ...
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" className="accent-blue-600" /> Permite
            mediação desde o início
          </label>
        </div>
      </Modal>
      ...
      {/* Chat */}
      <Modal
        open={modal === "chat" && !!activeService}
        onClose={() => setModal(null)}
        title={`Chat — ${activeProvider?.name}`}
        actions={
          <button className="px-4 py-2 rounded-xl bg-blue-600 text-white inline-flex items-center gap-2">
            <Send size={16} /> Enviar
          </button>
        }
        maxWidth="max-w-2xl"
      >
        <div className="h-72 border rounded-xl p-3 bg-gray-50 mb-3 overflow-auto text-sm text-gray-700">
          <div>
            <b>Mediador:</b> Olá! Como posso ajudar?
          </div>
          <div className="mt-2">
            <b>{activeProvider?.name}:</b> Posso atender esta semana.
          </div>
        </div>
        <input
        ...
        <label className="mt-3 flex items-center gap-2 text-sm">
          <input type="checkbox" className="accent-blue-600" /> Incluir mediador
          nesta conversa
        </label>
      </Modal>
      ...
      >
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button className="border rounded-xl p-3 text-left hover:bg-gray-50">
              <div className="font-medium">Express</div>
              <div className="text-xs text-gray-500">Cartão/Carteira</div>
            </button>
            <button className="border rounded-xl p-3 text-left hover:bg-gray-50">
              <div className="font-medium">Transferência</div>
              <div className="text-xs text-gray-500">IBAN/Comprovativo</div>
            </button>
            <button className="border rounded-xl p-3 text-left hover:bg-gray-50">
              <div className="font-medium">Pagamento por referência</div>
              <div className="text-xs text-gray-500">Multicaixa/Ref.</div>
            </button>
          </div>
          <input
            className="w-full border rounded-xl px-3 py-2.5"
            placeholder="Valor (AOA)"
            defaultValue={activeProvider ? activeProvider.priceFrom : ""}
          />
        </div>
      </Modal>
      ...
      {/* Acompanhar */}
      <Modal
        open={modal === "track" && !!activeService}
        onClose={() => setModal(null)}
        ...
        <ol className="relative border-s-2 border-blue-200 ps-5 space-y-4 text-sm">
          <li>
            ...
            <div className="text-gray-500">
              Recebemos o seu pedido e estamos a confirmar detalhes.
            </div>
          </li>
          <li>
            ...
            Prestador atribuído
            <div className="text-gray-500">
              Mediador confirmou {activeProvider?.name}.
            </div>
          </li>
          <li>
      ...
      {/* Avaliar */}
      <Modal
        open={modal === "rate" && !!activeService}
        onClose={() => setModal(null)}
        title={`Avaliar — ${activeProvider?.name}`}
        actions={
          <>
            <button
              onClick={() => setModal(null)}
      ...
      >
        <div className="space-y-3">
          ...
          <textarea
            className="w-full border rounded-xl p-3"
            rows={4}
            placeholder="Deixe o seu feedback para ajudar outros clientes"
          />
        </div>
      </Modal>
      ...
      {/* Reclamar */}
      <Modal
        open={modal === "complain" && !!activeService}
        onClose={() => setModal(null)}
        title="Reclamação formal via mediador"
        actions={
          <>
            <button
              onClick={() => setModal(null)}
      ...
      >
        <div className="space-y-3">
          ...
          />
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" className="accent-blue-600" /> Enviar cópia
            ao prestador
          </label>
        </div>
      </Modal>
      ...
      {/* Perfil do prestador */}
      <Modal
        open={modal === "provider" && !!activeProvider}
        onClose={() => setModal(null)}
        title={`Perfil — ${activeProvider?.name}`}
        maxWidth="max-w-3xl"
      >
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <div className="text-sm text-gray-600 mb-2">
              ...
              {activeProvider?.rating} ({activeProvider?.reviews}) •{" "}
              {activeProvider?.city}
            </div>
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-2">
                {activeProvider?.specialties.map((s) => (
                  <span
                    key={s}
                    className="text-xs bg-white px-2 py-1 rounded-xl border"
                  >
                    {s}
                  </span>
                ))}
            ...
            <div className="mt-4">
              ...
              <div className="text-lg font-extrabold">
                {currency(activeProvider?.priceFrom ?? 0)}
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {(
                activeProvider?.portfolio ?? [
                  { title: "Trabalho A", img: "" },
                  { title: "Trabalho B", img: "" },
                ]
              ).map((item, i) => (
                <div
                  key={i}
                  className="bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl h-24 grid place-items-center text-xs text-gray-500"
                >
                  {item.title}
                </div>
              ))}
        ...
        <div className="mt-5 flex gap-2">
          {activeService && (
            <button
              onClick={() =>
                openAction(activeService, activeProvider!, "quote")
              }
          ...
          {activeService && (
            <button
              onClick={() => openAction(activeService, activeProvider!, "chat")}
              className="border px-4 py-2 rounded-xl"
            >
              Chat
            </button>
